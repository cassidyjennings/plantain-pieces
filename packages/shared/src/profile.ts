...
/** Letters (any script), digits, spaces, underscore and hyphen. No leading/trailing
 * whitespace (callers should trim first; validation also rejects it defensively). */
...
/** Validates a candidate display name. Expects the caller to have trimmed it, but is
 * defensive about surrounding whitespace so a raw value can't slip through. */
export function validateDisplayName(raw: string): DisplayNameValidity {
  ...
  if (!DISPLAY_NAME_PATTERN.test(name)) return { valid: false, reason: 'INVALID_CHARS' };
  return { valid: true };
}
