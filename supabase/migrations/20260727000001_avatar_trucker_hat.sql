-- Add the 'trucker' hat to the avatar options.
--
-- The allowed-accessory list lives in TWO places: packages/shared/src/avatar.ts (ACCESSORY_SETS,
-- the authoritative check the Worker runs) and update_profile's own defense-in-depth copy below.
-- They must be kept in sync — without this migration the client would offer the hat, the shared
-- validator would accept it, and only the RPC would reject it with INVALID_AVATAR_CONFIG at save
-- time. Only the accessory list changes here; the rest of the function is carried over verbatim
-- from 20260720000001_relax_display_name.sql.
