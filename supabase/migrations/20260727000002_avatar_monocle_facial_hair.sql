-- Add the 'monocle' glasses option and the facial-hair options ('mustache', 'goatee') to the
-- avatar accessory lists.
--
-- Same sync obligation as 20260727000001: the allowed-accessory list lives in BOTH
-- packages/shared/src/avatar.ts (ACCESSORY_SETS) and update_profile's defense-in-depth copy
-- below, and a new option that only exists in the TS half is offered by the UI, accepted by the
-- shared validator, then rejected here with INVALID_AVATAR_CONFIG at save time. Only the
-- accessory list changes here; the rest of the function is carried over verbatim from
-- 20260727000001_avatar_trucker_hat.sql.
...
declare
  ...
  v_allowed jsonb := jsonb_build_object(
    ...
    'glasses', jsonb_build_array('none','round','shades','star','monocle'),
    'hair', jsonb_build_array('none','swoop','curls','mohawk','mustache','goatee')
  );
  v_base text;
